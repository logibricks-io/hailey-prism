# prism-browser (Node helper runtime)

The Node.js helper layer that runs inside the `prism-browser` Chromium browser. The browser exposes an `prism` runtime (tabs, CDP, snapshots, task spaces); this package bundles the agent-facing helpers that script that runtime.

```text
prism-browser (Chromium) -> globalThis.prism -> Playwright-style helper facades -> agent heredoc
```
...
The build emits a single ESM file `dist/out/index.js`. The prism-browser browser dispatches `prism-browser nodejs <<'EOF' ... EOF` heredocs to that bundle. Inside the heredoc, Playwright-style facades (`page`, `page.locator`, `browser`, `taskSpaces`, `site`, `fetch`, `cdp`) are preloaded.
...
```text
../../skills/prism-browser
```

Override with `PRISM_BROWSER_AGENT_WORKSPACE`:

```bash
PRISM_BROWSER_AGENT_WORKSPACE=/path/to/skill prism-browser nodejs <<'EOF'
...
src/
  ...
  browser-runtime.ts     bridge to globalThis.prism (CDP, sessions, events)
...
The top-level repo README has the full helper inventory and the task-space / control-handoff protocol. See also `../../skills/prism-browser/SKILL.md` for the agent-facing contract.
...
- Snapshot helpers use the browser runtime contract: `prism.snapshot({ scope, includeActionMarks, includeStableLocator })`.
- Public agent-facing helpers are object-style facades; internal implementation helpers remain camelCase.
- Site-specific reusable experience belongs under `skills/prism-browser/learnings/`, not in this package.
