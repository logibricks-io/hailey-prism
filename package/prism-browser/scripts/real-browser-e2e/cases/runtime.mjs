...
export function cdpJsHelpCase() {
  return `
    ...
    assertEqual(evalResult.result.value, "prism-lite helper e2e", "cdp evaluates in page");

    await assertRejects(
      () => cdp("PrismBrowser.MissingMethodForE2E"),
      "PrismBrowser.MissingMethodForE2E",
      "cdp reports unsupported methods"
    );

    const title = await page.evaluate(() => document.title);
    assertEqual(title, "prism-lite helper e2e", "js returns evaluated value");

    const wrappedReturn = await page.evaluate("const title = document.title; return title;");
    assertEqual(wrappedReturn, "prism-lite helper e2e", "js wraps top-level return statements");
    ...
    assertEqual(titleFromFunction, "prism-lite helper e2e", "js supports function input");
