...
import { prismSource } from "./prism-source.mjs";
...
export async function runRealBrowserE2e() {
  const keepTaskSpace =
    process.env.PRISM_BROWSER_REAL_E2E_KEEP === "1" ||
    process.env.PRISM_BROWSER_REAL_E2E_KEEP === "true";
  const onlyCases = new Set(
    (process.env.PRISM_BROWSER_REAL_E2E_ONLY || "")
      .split(",")
      .map((name) => name.trim())
      .filter(Boolean),
  );
  ...
  if (unknownOnlyCases.length > 0) {
    console.error(
      `Unknown PRISM_BROWSER_REAL_E2E_ONLY case(s): ${unknownOnlyCases.join(", ")}`,
    );
  ...
  async function runNodeBridgeSmoke(timeoutMs = 15000) {
    ...
    const marker = `PRISM_NODEJS_BRIDGE_SMOKE_${Date.now()}`;
    ...
    // output reaches stdout, and helperCount > 0 proves installPrismSdk ran (it sets the
    // console.log override and prism.helpers in the same call), so the override ran too.
    const source = `
      console.log(${JSON.stringify(marker)});
      console.log(JSON.stringify({
        prismType: typeof globalThis.prism,
        hasSendCDPMessage: typeof globalThis.prism?.sendCDPMessage,
        processVersion: process.version,
        helperCount: Object.keys(globalThis.prism?.helpers || {}).length
      }));
    `;
    try {
  ...
  async function runPrismCase(name, body, timeoutMs = 45000, options = {}) {
    const visible = options.visible !== false;
    if (visible) console.log(`-- ${name}`);
    const source = prismSource(body, {
      ...context,
  ...
  async function maybeRunPrismCase(name, body, timeoutMs = 45000) {
  ...
  try {
    ...
    await stat(prismBrowserSdkPath);

    tempDir = await mkdtemp(join(tmpdir(), "prism-browser-real-e2e-"));
    ...
    const taskName = `prism-lite real browser e2e ${Date.now()}-${Math.random()
      .toString(16)
      .slice(2)}`;
    ...
    await writeFile(uploadPath, "prism-browser upload fixture\n", "utf8");
    ...
    for (const testCase of e2eCases) {
      await maybeRunPrismCase(testCase.name, testCase.body());
    }
...
async function initializeE2eEnvironment(context, tempDir) {
  ...
  if (!ticketPageResponse.ok || !ticketPage.includes("PRISM STARLIGHT TOUR")) {
    throw new Error(
