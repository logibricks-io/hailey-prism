...
/**
 * Install a mock prism that auto-responds to every CDP send with { result: {} }.
 * Use the returned `calls` array to inspect what was sent, and `prism.onCDPMessage`
 * (set by rawCdp after the first send) to deliver events directly.
 */
function installAutoPrism(listTabsResult) {
  const calls = [];
...
/**
 * Install a mock prism with manual control over sendCDPMessage.
 * Returns the calls array; use prism.onCDPMessage after a send to deliver responses.
 */
function installManualPrism(listTabsResult) {
  const calls = [];
  const tabs = listTabsResult || {
    tabs: [{ targetId: "tab-1", active: true }],
  };
...
function fireEvent(method, params = {}, sessionId) {
  ...
  globalThis.prism.onCDPMessage(JSON.stringify(msg));
}
...
test("isBrowserRuntime returns false when prism is not installed", () => {
  delete globalThis.prism;
  assert.equal(isBrowserRuntime(), false);
});

test("isBrowserRuntime returns true when prism has sendCDPMessage", () => {
  installAutoPrism();
  try {
...
test("isBrowserRuntime returns false when prism lacks sendCDPMessage", () => {
  globalThis.prism = { listTabs() {} };
  try {
    assert.equal(isBrowserRuntime(), false);
  } finally {
    delete globalThis.prism;
...
test("browserCdp with explicit sessionId sends and resolves on matching response", async () => {
  const calls = installManualPrism();
  try {
...
test("handleMessage ignores unparseable JSON", async () => {
  ...
  try {
    ...
    globalThis.prism.onCDPMessage("not valid json{{{");
...
test("handleMessage ignores responses for unknown request ids", async () => {
  ...
  try {
    const p = browserCdp("Target.getVersion", {}, undefined, 5000);
    globalThis.prism.onCDPMessage(JSON.stringify({ id: 999999, result: {} }));
    const evts = drainBrowserEvents();
...
test("handleSendError rejects all pending requests with prism error", async () => {
  installManualPrism();
  try {
    const p1 = browserCdp("Target.getVersion", {}, undefined, 5000);
    const p2 = browserCdp("Browser.getVersion", {}, undefined, 5000);

    globalThis.prism.onSendCDPMessageError(
      "native text",
      "PRISM_TASK_SPACE_USER_IN_CONTROL",
    );

    await assert.rejects(p1, (err) => {
      assert.equal(err.error_code, "PRISM_TASK_SPACE_USER_IN_CONTROL");
      return true;
    });
