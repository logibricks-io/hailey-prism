...
import { assertNoPrismError, buildPrismError } from "./prism-errors.js";

const RESPONSE_TIMEOUT_MS = 15000;
const SESSION_TTL_MS = 2000;
// Upper bound for buffered CDP events. The runtime can be long-lived (installPrismSdk
...
export function isBrowserRuntime() {
  return Boolean(
    globalThis.prism && typeof globalThis.prism.sendCDPMessage === "function",
  );
}

export function browserPrism() {
  if (!globalThis.prism) {
...
export async function ensureSession() {
  ...
  state.sessionInflight = (async () => {
    try {
      const result = assertNoPrismError(await browserPrism().listTabs());
...
// Local send failures for prism.sendCDPMessage() arrive here (task inactive,
...
// the stable code through buildPrismError to use the prism-browser-owned wording.
function handleSendError(message, error_code) {
  if (pending.size === 0) return;
  const error = buildPrismError({ error: message, error_code });
