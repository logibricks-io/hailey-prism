...
function installAutoPrism(options = {}) {
  const calls = [];
  globalThis.prism = {
    async listTabs() {
...
function fireEvent(method, params = {}, sessionId = "sess-1") {
  globalThis.prism.onCDPMessage(JSON.stringify({ method, params, sessionId }));
}
...
test("waitForEvent('download') falls back to Page.setDownloadBehavior", async () => {
  const calls = installAutoPrism({ browserSetDownloadBehaviorError: true });
  try {
    const promise = waitForEvent("download", { timeout: 1000 });
    await new Promise((resolve) => setTimeout(resolve, 20));
    fireEvent("Page.downloadWillBegin", {
