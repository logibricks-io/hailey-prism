 ...
 * @param {{clearFirst?: boolean, timeout?: number}} [options] clearFirst defaults to true (Playwright fill always clears); clearFirst:false appends (prism-browser extension). timeout in milliseconds.
 * @returns {Promise<void>}
 */
...
async function installKeyProbe(key: string) {
  ...
  try {
    const result = await cdp("Runtime.evaluate", {
      expression: `(() => {
      window.__prismBrowserInputProbes ||= {};
      ...
      window.__prismBrowserInputProbes[${JSON.stringify(id)}] = probe;
      return true;
    })()`,
      returnByValue: true,
      awaitPromise: false,
...
async function finishKeyProbe(
  ...
  try {
    const result = await cdp("Runtime.evaluate", {
      expression: `(() => {
      const probes = window.__prismBrowserInputProbes || {};
...
function canProbeInputFallback() {
  return Boolean((globalThis as any).prism?.sendCDPMessage);
}
