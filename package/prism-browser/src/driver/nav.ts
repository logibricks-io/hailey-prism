...
import { assertNoPrismError } from "../prism-errors.js";
...
export async function listTabs(
  ...
  const result = assertNoPrismError(await browserPrism().listTabs(), "listTabs");
...
export async function newTab(url = "about:blank") {
  const result = assertNoPrismError(await browserPrism().createTab(url), "newTab");
  if (!result.targetId) {
