...
test("screenshot skips page metric JavaScript while a native dialog is pending", async () => {
  ...
  try {
    await withCdpRuntime(async ({ runtime, sent }) => {
      ...
      await screenshot({ path: "/tmp/prism-browser-dialog-shot.png" });
  ...
  assert.equal(writes[0].path, "/tmp/prism-browser-dialog-shot.png");
});

test("screenshot creates a missing parent directory", async () => {
  const tempDir = await mkdtemp(join(tmpdir(), "prism-browser-observe-"));
