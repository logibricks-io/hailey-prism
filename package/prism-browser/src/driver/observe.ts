...
import { buildPrismError } from "../prism-errors.js";
...
export async function snapshotRaw(options: SnapshotOptions = {}) {
  let result;
  try {
    result = await browserPrism().snapshot(options);
  } catch (err) {
    // prism.snapshot rejects directly (it never resolves with { error }), so it never
    // reached buildPrismError — the single birthplace that records a hard stop for the
    // output sink and swaps native wording for prism-browser's owned guidance. Route the
    // rejection through it so a swallowed snapshot hard stop collapses like every other
    // prism error instead of leaking repeated native text.
    throw buildPrismError(err, "snapshot");
  }
...
export async function screenshot(options: ScreenshotOptions = {}) {
  const path =
    options.path ??
    join(tmpdir(), `prism-browser-shot-${process.pid}-${++screenshotSeq}.png`);
