...
test("click resolves selector offsets without the public elementEval helper", async () => {
  ...
  const restore = setOverrides({
    cdpOverride(method, params, sessionId) {
      ...
      if (
        method === "Runtime.evaluate" &&
        params.objectGroup === "prism-browser"
      ) {
...
test("click triggers probe fallback when CDP click is not trusted", async () => {
  const originalPrism = globalThis.prism;
  globalThis.prism = { sendCDPMessage: () => {} };
  let probeEvaluateCount = 0;
  const evaluateExpressions = [];
  const restore = setOverrides({
    cdpOverride(method, params, sessionId) {
      if (method === "Runtime.evaluate") {
        // Probe calls contain __prismBrowserInputProbes; element resolution does not
        if (params.expression?.includes("__prismBrowserInputProbes")) {
          probeEvaluateCount++;
        ...
        if (params.objectGroup === "prism-browser") {
          return { result: { objectId: "object-1" } };
        }
  ...
  try {
    ...
    restore();
    if (originalPrism === undefined) delete globalThis.prism;
    else globalThis.prism = originalPrism;
  }
