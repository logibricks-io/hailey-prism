...
function installAutoPrism(resultFor = () => ({})) {
  const calls = [];
  globalThis.prism = {
    async listTabs() {
    ...
    sendCDPMessage(payload) {
      ...
      calls.push(parsed);
      setTimeout(() => {
        if (!globalThis.prism?.onCDPMessage) return;
        const outcome =
        ...
        globalThis.prism.onCDPMessage(JSON.stringify(message));
      }, 0);
    },
  };
  return calls;
...
function fireEvent(method, params = {}) {
  globalThis.prism.onCDPMessage(JSON.stringify({ method, params }));
}
