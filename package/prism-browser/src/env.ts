...
export function agentWorkspace() {
  if (process.env.PRISM_BROWSER_AGENT_WORKSPACE) {
    return resolvePath(process.env.PRISM_BROWSER_AGENT_WORKSPACE);
  }

  const bundledSkill = resolve(SRC_DIR, "prism-browser");
  if (existsSync(bundledSkill)) {
    return bundledSkill;
  }

  return resolve(REPO_ROOT, "..", "..", "skills", "prism-browser");
}
