  ...
  await withPrism(
    {
      ...
      useTaskSpace(taskId) {
        calls.push(["useTaskSpace", taskId]);
        // Native attaches the stable code; resolvePrismError overrides the live
        // text with prism-browser's owned PRISM_TASK_SPACE_USER_IN_CONTROL guidance.
        return {
...
// The probe in waitForAgentControl keys on prism.snapshot()'s rejection carrying
// error_code === PRISM_TASK_SPACE_USER_IN_CONTROL (the documented contract), not on
...
function taskSpacePrism(snapshot) {
  return {
    async listTaskSpaces() {
      return {
