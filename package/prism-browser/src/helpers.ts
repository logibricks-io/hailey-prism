...
import { assertNoPrismError, isPrismUserControlError } from "./prism-errors.js";
...
export async function listTaskSpaces() {
  ...
  if (!prism || typeof prism.listTaskSpaces !== "function") {
    throw new Error("listTaskSpaces requires prism.listTaskSpaces");
  }
  return normalizeTaskSpaces(
    assertNoPrismError(await prism.listTaskSpaces(), "listTaskSpaces"),
  );
}
 ...
 *
 * Keep this table in sync with the one in skills/prism-browser/SKILL.md.
 */
...
export async function switchTaskSpace(nameOrId) {
  const prism = globalThis.prism;
  if (!prism || typeof prism.useTaskSpace !== "function") {
    throw new Error("switchTaskSpace requires prism.useTaskSpace");
  }
  ...
  return selectTaskSpace(prism, space, "switchTaskSpace");
}
...
export async function newTaskSpace(name) {
  const prism = globalThis.prism;
  if (!prism || typeof prism.createTaskSpace !== "function") {
    throw new Error("newTaskSpace requires prism.createTaskSpace");
  }
  const created = normalizeTaskSpace(
    assertNoPrismError(await prism.createTaskSpace(name), "newTaskSpace"),
  );
  if (!created) {
    throw new Error("newTaskSpace returned an invalid task space");
  }
  taskSpaceNumericId(created, "newTaskSpace");
  return selectTaskSpace(prism, created, "newTaskSpace");
}

/**
 * Use an existing agent-owned task space, or create it when missing. User-owned
 * spaces are selected but not claimed (the PRISM_TASK_SPACE_USER_IN_CONTROL error
...
export async function useOrCreateTaskSpace(nameOrId) {
  ...
  if (isAgentOwned(existing.ownership)) {
    return selectTaskSpace(globalThis.prism, existing, "useOrCreateTaskSpace");
  }
  if (existing.ownership === "user") {
    // Don't claim user-owned spaces here. Select it as-is; the user stays in
    // control, so PRISM_TASK_SPACE_USER_IN_CONTROL surfaces (as prism-browser's owned
 ...
 * prism.claimTaskSpace, then selects it.
 * @param {string|number} nameOrId Task space id or name.
 * @returns {Promise<{taskId:string,id:number,name:string,createdBy?:string,ownership?:string,recentTabTitles?:string[]}>}
 */
...
async function claimResolvedTaskSpace(space, op = "claimTaskSpace") {
  const prism = globalThis.prism;
  if (!prism || typeof prism.claimTaskSpace !== "function") {
    throw new Error(`${op} requires prism.claimTaskSpace`);
  }
  const id = taskSpaceNumericId(space, op);
  const claimed = normalizeTaskSpace(
    assertNoPrismError(await prism.claimTaskSpace(id, space.name), op),
  );
  if (!claimed) {
    throw new Error(`${op} returned an invalid task space`);
  }
  taskSpaceNumericId(claimed, op);
  return selectTaskSpace(prism, claimed, op);
}

async function selectTaskSpace(prism, space, op: string) {
  if (!prism || typeof prism.useTaskSpace !== "function") {
    throw new Error(`${op} requires prism.useTaskSpace`);
  }
  assertNoPrismError(await prism.useTaskSpace(taskSpaceNumericId(space, op)), op);
  return space;
}
...
async function selectTaskSpaceIfProvided(
  ...
  await selectTaskSpace(prism, match, op);
}
...
export async function completeTaskSpace(
  ...
  const prism = globalThis.prism;
  if (!prism) {
    throw new Error("completeTaskSpace requires prism runtime");
  }
  const spaces = await listTaskSpaces();
  ...
  if (options.keep) {
    ...
    await selectTaskSpace(prism, match, "completeTaskSpace");
    if (typeof prism.completeTaskSpace !== "function") {
      throw new Error("completeTaskSpace requires prism.completeTaskSpace");
    }
    assertNoPrismError(await prism.completeTaskSpace(), "completeTaskSpace");
  } else {
    if (match.ownership === "user") {
    ...
    if (typeof prism.closeTaskSpace !== "function") {
      throw new Error("completeTaskSpace requires prism.closeTaskSpace");
    }
    assertNoPrismError(await prism.closeTaskSpace(), "completeTaskSpace");
  }
  return { done: true };
}
...
export async function handOffTaskSpace(nameOrId?: string | number) {
  const prism = globalThis.prism;
  if (!prism || typeof prism.handOffTaskSpace !== "function") {
    throw new Error("handOffTaskSpace requires prism.handOffTaskSpace");
  }
  if (nameOrId !== undefined) {
    ...
    await selectTaskSpace(prism, match, "handOffTaskSpace");
  }
  assertNoPrismError(await prism.handOffTaskSpace(), "handOffTaskSpace");
  return { done: true };
}
...
export async function takeOverTaskSpace(nameOrId?: string | number) {
  const prism = globalThis.prism;
  if (!prism || typeof prism.takeOverTaskSpace !== "function") {
    throw new Error("takeOverTaskSpace requires prism.takeOverTaskSpace");
  }
  await selectTaskSpaceIfProvided(prism, nameOrId, "takeOverTaskSpace");
  assertNoPrismError(await prism.takeOverTaskSpace(), "takeOverTaskSpace");
}

/**
 * Probe whether the agent currently holds control of the active task space.
 * Module-private; used by waitForAgentControl. Uses prism.snapshot, which
 * rejects under user-control (per prism-bindings spec) — a reliable
...
async function probeAgentControl() {
  const prism = globalThis.prism;
  if (!prism || typeof prism.snapshot !== "function") return false;
  try {
    await prism.snapshot({ maxResultLength: 1 });
    return true;
  } catch (err) {
    if (isPrismUserControlError(err)) return false;
    throw err;
  }
}
...
export async function waitForAgentControl(
  ...
  const prism = globalThis.prism;
  if (!prism) {
    throw new Error("waitForAgentControl requires prism runtime");
  }
  await selectTaskSpaceIfProvided(prism, nameOrId, "waitForAgentControl");
