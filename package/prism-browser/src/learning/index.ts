...
export function wrapBrowserTool(source, args: any = {}) {
  return `(async () => { const __prismBrowserTool = ${source}; return await __prismBrowserTool(${JSON.stringify(args || {})}); })()`;
}

export { learningEntry };

function siteSkillNotFoundError(siteId, searchedRoot) {
  const workspace = process.env.PRISM_BROWSER_AGENT_WORKSPACE || "unset";
  const lines = [
    `site skill not found: ${JSON.stringify(siteId)}`,
    `  searched: ${searchedRoot}`,
    `  PRISM_BROWSER_AGENT_WORKSPACE: ${workspace}`,
