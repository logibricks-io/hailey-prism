...
// A minimal native prism whose only method reports a hard stop, the same shape the real
// bindings return when the user holds (or has not handed over) the task space. The
// `listTaskSpaces` helper lifts it through assertNoPrismError -> buildPrismError, which is
// where the sink is told a hard stop occurred.
function hardStopPrism(error_code) {
  return {
    calls: 0,
...
// A native prism whose `snapshot` REJECTS with a hard-stop code — the shape the real
// bindings use under user control (helpers.ts probeAgentControl relies on it). driver/
// observe.ts calls browserPrism().snapshot() directly, so the rejection only reaches the
// sink if snapshot() routes it through buildPrismError.
function snapshotHardStopPrism(error_code) {
  return {
    calls: 0,
    async snapshot() {
      this.calls += 1;
...
async function runScript(code, prism) {
  const previous = globalThis.prism;
  if (prism === undefined) {
...
test("a swallowed user-control hard stop discards all output and prints the guidance once", async () => {
  const prism = hardStopPrism("PRISM_TASK_SPACE_USER_IN_CONTROL");
  ...
  assert.ok(prism.calls >= 3, "every iteration should have hit the hard stop");
});

test("an inactive / unassigned task space is also a hard stop", async () => {
  const prism = hardStopPrism("PRISM_TASK_SPACE_INACTIVE");
  const result = await runScript(
    `
      try {
        await taskSpaces.list();
...
test("a swallowed snapshot hard stop (rejected, not resolved) also collapses to one message", async () => {
  ...
  // assertNoPrismError; the collapse only works if snapshot() rebuilds it via buildPrismError.
  const prism = snapshotHardStopPrism("PRISM_TASK_SPACE_USER_IN_CONTROL");
  const result = await runScript(
    `
      for (const site of ["a", "b", "c"]) {
        console.log("visiting " + site);
