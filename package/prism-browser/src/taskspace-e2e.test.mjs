...
async function runTaskspaceScript(prism, code) {
...
test("taskspace e2e reuses an existing agent-owned task space", async () => {
  ...
  assert.deepEqual(prism.calls, [["listTaskSpaces"], ["useTaskSpace", 7]]);
});
...
test("taskspace e2e useOrCreateTaskSpace selects user-owned spaces without claiming and surfaces the owned user-control guidance", async () => {
  ...
  // Native rejects with error_code PRISM_TASK_SPACE_USER_IN_CONTROL, so the agent
  // sees prism-browser's owned guidance block, not the raw native text.
  await assert.rejects(
    () =>
      runTaskspaceScript(prism, `await taskSpaces.useOrCreate("checkout-flow")`),
...
test("taskspace e2e exposes taskSpaces facade", async () => {
  ...
  const result = await runTaskspaceScript(
    ...
    console.log(JSON.stringify({
      ...
      rawClaimType: typeof prism.claimTaskSpace
    }));
  `,
  );
...
test("taskspace e2e rejects explicit use of a user-owned task space", async () => {
  ...
  await assert.rejects(
    () => runTaskspaceScript(prism, `await taskSpaces.switch("checkout-flow")`),
    /switchTaskSpace requires an agent-owned task space/,
  );
  assert.deepEqual(prism.calls, [["listTaskSpaces"]]);
});
...
test("taskspace e2e surfaces newTaskSpace binding errors", async () => {
  ...
  await assert.rejects(
    () => runTaskspaceScript(prism, `await taskSpaces.new("checkout-flow")`),
    /newTaskSpace: Task space already exists: checkout-flow/,
  );
  assert.deepEqual(prism.calls, [["createTaskSpace", "checkout-flow"]]);
});
