...
test("composeNotice renders an available update with prefix, versions, and hint", () => {
  ...
  assert.match(line, /prism lite 0\.4\.4\.0 is available/);
  assert.match(line, /current 0\.4\.3\.0/);
  assert.match(line, /run: prism-browser upgrade in your shell/);
  assert.match(line, /re-read the prism-browser skill/);
});
...
test("composeNotice falls back to a generic phrase without a usable latest version", () => {
  assert.match(
    ...
    /an prism lite update is available/,
  );
...
test("composeNotice treats blank version strings as absent", () => {
  ...
  // A blank latestVersion degrades to the generic phrase, never "prism lite  is available".
  const line = composeNotice({
    currentVersion: "0.4.3.0",
    updateAvailable: true,
    latestVersion: "",
  });
  assert.match(line, /an prism lite update is available/);
...
test("noticeSuppressed honors the opt-out var and CI", () => {
  assert.equal(noticeSuppressed({ PRISM_BROWSER_NO_UPDATE_NOTIFIER: "1" }), true);
...
// emitUpdateNotice (the installPrismSdk wiring point) --------------------------
...
test("emitUpdateNotice hands one line to emit when the bridge reports an update", async () => {
  ...
  emitUpdateNotice(prism, emit, {});
  await flushMicrotasks();
  assert.equal(lines.length, 1);
  assert.ok(lines[0].startsWith(NOTICE_PREFIX));
  assert.match(lines[0], /run: prism-browser upgrade in your shell/);
});
...
test("emitUpdateNotice stays silent when there is no prism bridge at all", async () => {
  const { lines, emit } = collect();
...
test("emitUpdateNotice is suppressed in CI even when an update is available", async () => {
  ...
  const { lines, emit } = collect();
  emitUpdateNotice(prism, emit, { CI: "true" });
  await flushMicrotasks();
  assert.equal(lines.length, 0);
});
