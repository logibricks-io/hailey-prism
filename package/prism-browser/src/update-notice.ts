/**
 * "prism lite has an update" hint for the agent-facing runtime.
 *
 * Loosely modeled on lark cli's `_notice` — surface one tagged line telling the
 * agent a newer prism lite exists — but deliberately WITHOUT lark's cache. lark
 * persists a throttled state file because every check is an HTTP round-trip to
 * npm, an expensive cost worth amortizing. Our source is `prism.getBrowserVersion()`:
 ...
 *
 * The version source is injected — in production `installPrismSdk` builds it from the
 * app's `prism` bridge. On older app builds without `getBrowserVersion`, that source
 * yields null and the check degrades to "no update". The remedy is *not* a symmetric
 * `prism.upgradeBrowser()` bridge call — the app exposes that half as the native CLI
 * subcommand `prism-browser upgrade`, so the composed line tells the agent to run it as
 * a shell command.
 *
...
/** Raw shape the client `prism.getBrowserVersion()` is expected to return. */
...
export const NOTICE_PREFIX = "[prism-browser:notice]";
...
export function noticeSuppressed(
  ...
  return Boolean(env.PRISM_BROWSER_NO_UPDATE_NOTIFIER || env.CI);
}
...
export function composeNotice(
  ...
  const target = isNonEmptyString(info.latestVersion)
    ? `prism lite ${info.latestVersion}`
    : "an prism lite update";
  const urgency = info.mandatory === true ? "is required" : "is available";
  return `${NOTICE_PREFIX} ${target} ${urgency} (current ${info.currentVersion}) — run: prism-browser upgrade in your shell, then re-read the prism-browser skill`;
}
...
/**
 * The one real entry point: given the app's injected `prism` bridge (or none, on older
 * builds), fire the check and hand the resulting line to `emit`. Fire-and-forget —
 * `installPrismSdk()` calls this without awaiting it, so the check runs concurrently with
...
export function emitUpdateNotice(
  prism: { getBrowserVersion?: VersionSource } | null | undefined,
  emit: (line: string) => void,
  env?: NodeJS.ProcessEnv,
): void {
  updateNoticeLine({
    source: () => prism?.getBrowserVersion?.() ?? Promise.resolve(null),
    env,
  })
    .then((line) => {
      if (line) emit(line);
