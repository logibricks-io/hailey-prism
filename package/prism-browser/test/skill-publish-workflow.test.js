...
const workflowUrl = new URL(
  "../../../.github/workflows/publish-prism-browser-skill.yml",
  import.meta.url,
);

test("publishes prism-browser to ClawHub and SkillHub from an exact SemVer tag", () => {
  ...
  assert.match(workflow, /^\s+SKILLHUB_SLUG: prism-browser$/m);
  assert.doesNotMatch(workflow, /prism-browser-user-/);
  assert.match(workflow, /skillhub publish[\s\S]*--dry-run/);
  assert.match(workflow, /skillhub publish[\s\S]*--version "\$VERSION"/);
  assert.match(workflow, /skills\/prism-browser/);
});
