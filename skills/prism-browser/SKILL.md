...
prism-browser gives AI agents a CLI-accessible Node.js runtime, with built-in helpers — snapshotText, click, js, cdp, and more — that agents call directly inside JS scripts to observe pages, interact with UI, evaluate browser-side JavaScript, and drive a real browser for any web automation task.

For setup, install, or connection problems, read `references/install.md`.

Use the `Bash` tool to run all browser operations via `prism-browser nodejs <<'EOF' ... EOF` heredoc. Do not write code to a `.js` file first.
...
The heredoc body runs as a Node.js script that controls the selected prism-browser task space. All prism-browser helpers are preloaded into that script.
...
A task space is an **isolated browsing context** that prism-browser provides for AI Agents. Each task space has its own set of tabs but **inherits the current user's login state** by default, so Agents can operate on authenticated sites without competing with or disturbing the user's normal browser windows.
...
prism-browser has three main workflows. Pick the workflow that fits the page and task before acting.
...
- When the user explicitly asks to use prism-browser, assume both `prism-browser` and the repo runtime are ready. Do not pre-check `which prism-browser`, `node -v`, package metadata, or help output. Only investigate environment issues if the first run produces an error.
- If the first run reports `command not found` / a missing environment (most likely prism lite isn't installed yet), or the user explicitly asks to install prism lite, first read `references/install.md` and follow its flow to complete the install, then return to the original task — do not give up, and do not keep retrying the same heredoc.
